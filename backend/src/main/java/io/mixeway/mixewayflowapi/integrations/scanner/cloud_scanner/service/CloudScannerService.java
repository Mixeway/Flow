...
public class CloudScannerService {
    ...
    public String fetchVulnerabilityFindings(String projectId, String wizAuthToken) {
        String query = "query VulnerabilityFindings($project_name: [String!]) { " +
                "vulnerabilityFindings(first: 800 filterBy: {status: [OPEN, IN_PROGRESS], assetType: VIRTUAL_MACHINE, severity: [HIGH, CRITICAL], hasExploit: true, hasFix: true, detectionMethod: [PACKAGE, OS], projectId: $project_name} orderBy: {direction: DESC}) { " +
